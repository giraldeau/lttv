...
static PossibleCompletion* attempt_homedir_completion(gchar* text_to_complete,
						      CompletionState *cmpl_state);
#endif
static PossibleCompletion* attempt_dir_completion(CompletionState *cmpl_state);
...
static void gtk_dir_selection_set_property  (GObject         *object,
					      guint            prop_id,
					      const GValue    *value,
					      GParamSpec      *pspec);
static void gtk_dir_selection_get_property  (GObject         *object,
					      guint            prop_id,
					      GValue          *value,
					      GParamSpec      *pspec);
static void gtk_dir_selection_init          (GtkDirSelection      *filesel);
static void gtk_dir_selection_finalize      (GObject               *object);
static void gtk_dir_selection_destroy       (GtkObject             *object);
static void gtk_dir_selection_map           (GtkWidget             *widget);
static gint gtk_dir_selection_key_press     (GtkWidget             *widget,
					      GdkEventKey           *event,
					      gpointer               user_data);
static gint gtk_dir_selection_insert_text   (GtkWidget             *widget,
					      const gchar           *new_text,
					      gint                   new_text_length,
					      gint                  *position,
					      gpointer               user_data);
static void gtk_dir_selection_update_fileops (GtkDirSelection     *filesel);

static void gtk_dir_selection_file_activate (GtkTreeView       *tree_view,
					      GtkTreePath       *path,
					      GtkTreeViewColumn *column,
					      gpointer           user_data);
static void gtk_dir_selection_file_changed  (GtkTreeSelection  *selection,
					      gpointer           user_data);
static void gtk_dir_selection_dir_activate  (GtkTreeView       *tree_view,
					      GtkTreePath       *path,
					      GtkTreeViewColumn *column,
					      gpointer           user_data);
static void gtk_dir_selection_dir_changed  (GtkTreeSelection  *selection,
					      gpointer           user_data);
static void gtk_dir_selection_populate      (GtkDirSelection      *fs,
					      gchar                 *rel_path,
					      gboolean               try_complete,
					      gboolean               reset_entry);
static void gtk_dir_selection_abort         (GtkDirSelection      *fs);

static void gtk_dir_selection_update_history_menu (GtkDirSelection       *fs,
						    gchar                  *current_dir);
 ...
 */
static int
translate_win32_path (GtkDirSelection *filesel)
{
  int updated = 0;
  const gchar *path;
...
gtk_dir_selection_get_type (void)
{
  ...
  if (!file_selection_type)
	...
	NULL,		/* base_init */
	NULL,		/* base_finalize */
	(GClassInitFunc) gtk_dir_selection_class_init,
	NULL,		/* class_finalize */
	NULL,		/* class_data */
	...
	(GInstanceInitFunc) gtk_dir_selection_init,
      ...
      file_selection_type =
	g_type_register_static (GTK_TYPE_DIALOG, "GtkDirSelection",
				&filesel_info, 0);
    }
...
gtk_dir_selection_class_init (GtkDirSelectionClass *class)
{
  ...
  gobject_class->finalize = gtk_dir_selection_finalize;
  gobject_class->set_property = gtk_dir_selection_set_property;
  gobject_class->get_property = gtk_dir_selection_get_property;
  ...
  widget_class->map = gtk_dir_selection_map;
}
...
static void gtk_dir_selection_set_property (GObject         *object,
					     ...
					     GParamSpec      *pspec)
{
  GtkDirSelection *filesel;

  filesel = GTK_DIR_SELECTION (object);

  switch (prop_id)
    {
    case PROP_FILENAME:
      gtk_dir_selection_set_filename (filesel,
                                       g_value_get_string (value));
      break;
    case PROP_SHOW_FILEOPS:
      if (g_value_get_boolean (value))
	 gtk_dir_selection_show_fileop_buttons (filesel);
      else
	 gtk_dir_selection_hide_fileop_buttons (filesel);
      break;
    case PROP_SELECT_MULTIPLE:
      gtk_dir_selection_set_select_multiple (filesel, g_value_get_boolean (value));
      break;
    default:
					     ...
					     GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_SHOW_FILEOPS:
      ...
      break;
    case PROP_SELECT_MULTIPLE:
      g_value_set_boolean (value, gtk_dir_selection_get_select_multiple (filesel));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
static void
gtk_dir_selection_init (GtkDirSelection *filesel)
{
  ...
  g_signal_connect (filesel->dir_list, "row_activated",
		    G_CALLBACK (gtk_dir_selection_dir_activate), filesel);
  g_signal_connect (gtk_tree_view_get_selection (GTK_TREE_VIEW (filesel->dir_list)), "changed",
		    G_CALLBACK (gtk_dir_selection_dir_changed), filesel);
  ...
  g_signal_connect (filesel->file_list, "row_activated",
		    G_CALLBACK (gtk_dir_selection_file_activate), filesel);
  g_signal_connect (gtk_tree_view_get_selection (GTK_TREE_VIEW (filesel->file_list)), "changed",
		    G_CALLBACK (gtk_dir_selection_file_changed), filesel);
  ...
  g_signal_connect (filesel->selection_entry, "key_press_event",
		    G_CALLBACK (gtk_dir_selection_key_press), filesel);
  g_signal_connect (filesel->selection_entry, "insert_text",
		    G_CALLBACK (gtk_dir_selection_insert_text), NULL);
  g_signal_connect_swapped (filesel->selection_entry, "changed",
			    G_CALLBACK (gtk_dir_selection_update_fileops), filesel);
  ...
  if (!cmpl_state_okay (filesel->cmpl_state))
    ...
    {
      gtk_dir_selection_populate (filesel, "", FALSE, TRUE);
    }
...
static void
dnd_really_drop  (GtkWidget *dialog, gint response_id, GtkDirSelection *fs)
{
  gchar *filename;
  ...
  if (response_id == GTK_RESPONSE_YES)
    {
      ...
      gtk_dir_selection_set_filename (fs, filename);
    }
		   ...
		   guint             time)
{
  ...
  if ((hostname == NULL) ||
    ...
    gtk_dir_selection_set_filename (GTK_DIR_SELECTION (widget),
				     filename);
  else
    {
      GtkWidget *dialog;
		    ...
		    GtkDirSelection *filesel)
{
  ...
  file = gtk_dir_selection_get_filename (filesel);
...
static void
file_selection_setup_dnd (GtkDirSelection *filesel)
{
  GtkWidget *eventbox;
...
gtk_dir_selection_new (const gchar *title)
{
  ...
  filesel = g_object_new (GTK_TYPE_DIR_SELECTION, NULL);
...
void
gtk_dir_selection_show_fileop_buttons (GtkDirSelection *filesel)
{
  g_return_if_fail (GTK_IS_DIR_SELECTION (filesel));
  ...
  if (!filesel->fileop_c_dir) 
    {
      ...
      g_signal_connect (filesel->fileop_c_dir, "clicked",
			G_CALLBACK (gtk_dir_selection_create_dir),
			filesel);
  ...
  if (!filesel->fileop_del_file) 
    {
      ...
      g_signal_connect (filesel->fileop_del_file, "clicked",
			G_CALLBACK (gtk_dir_selection_delete_file),
			filesel);
      //      gtk_box_pack_start (GTK_BOX (filesel->button_area), 
  ...
  if (!filesel->fileop_ren_file)
    {
      ...
      g_signal_connect (filesel->fileop_ren_file, "clicked",
			G_CALLBACK (gtk_dir_selection_rename_file),
			filesel);
      //      gtk_box_pack_start (GTK_BOX (filesel->button_area), 
  ...
  gtk_dir_selection_update_fileops (filesel);
  
  g_object_notify (G_OBJECT (filesel), "show_fileops");
}

void       
gtk_dir_selection_hide_fileop_buttons (GtkDirSelection *filesel)
{
  g_return_if_fail (GTK_IS_DIR_SELECTION (filesel));
...
/**
 * gtk_dir_selection_set_filename:
 * @filesel: a #GtkDirSelection.
 ...
 **/
void
gtk_dir_selection_set_filename (GtkDirSelection *filesel,
				 const gchar      *filename)
{
  ...
  gtk_dir_selection_populate (filesel, buf, FALSE, TRUE);
...
/**
 * gtk_dir_selection_get_filename:
 * @filesel: a #GtkDirSelection
 * 
...
gtk_dir_selection_get_filename (GtkDirSelection *filesel)
{
  ...
  g_return_val_if_fail (GTK_IS_DIR_SELECTION (filesel), nothing);
...
void
gtk_dir_selection_complete (GtkDirSelection *filesel,
			     const gchar      *pattern)
{
  g_return_if_fail (GTK_IS_DIR_SELECTION (filesel));
  ...
  gtk_dir_selection_populate (filesel, (gchar*) pattern, TRUE, TRUE);
}
...
static void
gtk_dir_selection_map (GtkWidget *widget)
{
  GtkDirSelection *filesel = GTK_DIR_SELECTION (widget);

  /* Refresh the contents */
  gtk_dir_selection_populate (filesel, "", FALSE, FALSE);
  
  GTK_WIDGET_CLASS (parent_class)->map (widget);
}

static void
gtk_dir_selection_finalize (GObject *object)
{
  GtkDirSelection *filesel = GTK_DIR_SELECTION (object);
...
static void
gtk_dir_selection_fileop_error (GtkDirSelection *fs,
				 gchar            *error_message)
{
  GtkWidget *dialog;
...
static void
gtk_dir_selection_fileop_destroy (GtkWidget *widget,
				   gpointer   data)
{
  GtkDirSelection *fs = data;

  g_return_if_fail (GTK_IS_DIR_SELECTION (fs));
...
static void
gtk_dir_selection_fileop_entry_changed (GtkEntry   *entry,
					 GtkWidget  *button)
{
...
static void
gtk_dir_selection_create_dir_confirmed (GtkWidget *widget,
					 gpointer   data)
{
  GtkDirSelection *fs = data;
  const gchar *dirname;
  ...
  if (error)
	...
	buf = g_strdup_printf (_("Error creating folder \"%s\": %s\n%s"), dirname, error->message,
      ...
      gtk_dir_selection_fileop_error (fs, buf);
      g_error_free (error);
      goto out;
    }
 ...
 out:
  ...
  gtk_dir_selection_populate (fs, "", FALSE, FALSE);
}
  
static void
gtk_dir_selection_create_dir (GtkWidget *widget,
			       gpointer   data)
{
  GtkDirSelection *fs = data;
  GtkWidget *label;
  ...
  g_signal_connect (dialog, "destroy",
		    G_CALLBACK (gtk_dir_selection_fileop_destroy),
		    fs);
  ...
  g_signal_connect (button, "clicked",
		    G_CALLBACK (gtk_dir_selection_create_dir_confirmed),
		    fs);
  g_signal_connect (fs->fileop_entry, "changed",
                    G_CALLBACK (gtk_dir_selection_fileop_entry_changed),
		    button);
...
static void
gtk_dir_selection_delete_dir_response (GtkDialog *dialog, 
                                         gint       response_id,
                                         gpointer   data)
{
  GtkDirSelection *fs = data;
 ...
 out:
  ...
  gtk_widget_destroy (fs->fileop_dialog);
  gtk_dir_selection_populate (fs, "", FALSE, TRUE);
}

static void
gtk_dir_selection_delete_file (GtkWidget *widget,
				gpointer   data)
{
  GtkDirSelection *fs = data;
  GtkWidget *dialog;
  ...
  g_signal_connect (dialog, "response",
                    G_CALLBACK (gtk_dir_selection_delete_dir_response),
                    fs);
...
static void
gtk_dir_selection_rename_dir_confirmed (GtkWidget *widget,
					  gpointer   data)
{
  GtkDirSelection *fs = data;
  gchar *buf;
...
static void
gtk_dir_selection_rename_file (GtkWidget *widget,
				gpointer   data)
{
  GtkDirSelection *fs = data;
  GtkWidget *label;
  ...
  g_signal_connect (button, "clicked",
		    G_CALLBACK (gtk_dir_selection_rename_dir_confirmed),
		    fs);
  g_signal_connect (fs->fileop_entry, "changed",
		    G_CALLBACK (gtk_dir_selection_fileop_entry_changed),
		    button);
...
static gint
gtk_dir_selection_insert_text (GtkWidget   *widget,
				const gchar *new_text,
				gint         new_text_length,
				gint        *position,
				gpointer     user_data)
...
static void
gtk_dir_selection_update_fileops (GtkDirSelection *fs)
{
  gboolean sensitive;
			      ...
			      gpointer     user_data)
{
  ...
  if ((event->keyval == GDK_Tab || event->keyval == GDK_KP_Tab) &&
    ...
    {
      fs = GTK_DIR_SELECTION (user_data);
#ifdef G_WITH_CYGWIN
      translate_win32_path (fs);
#endif
      text = g_strdup (gtk_entry_get_text (GTK_ENTRY (fs->selection_entry)));

      gtk_dir_selection_populate (fs, text, TRUE, TRUE);
...
static void
gtk_dir_selection_history_callback (GtkWidget *widget,
				     gpointer   data)
{
  GtkDirSelection *fs = data;
  HistoryCallbackArg *callback_arg;
  ...
  while (list) {
    ...
    if (callback_arg->menu_item == widget)
      {
	gtk_dir_selection_populate (fs, callback_arg->directory, FALSE, FALSE);
	break;
      }
...
static void 
gtk_dir_selection_update_history_menu (GtkDirSelection *fs,
  ...
  for (i = dir_len; i >= 0; i--)
      ...
      if ( (current_dir[i] == G_DIR_SEPARATOR) || (i == dir_len))
	{
	  ...
	  g_signal_connect (menu_item, "activate",
			    G_CALLBACK (gtk_dir_selection_history_callback),
			    fs);
...
static void
gtk_dir_selection_file_activate (GtkTreeView       *tree_view,
				  GtkTreePath       *path,
				  GtkTreeViewColumn *column,
				  gpointer           user_data)
{
  GtkDirSelection *fs = GTK_DIR_SELECTION (user_data);
...
static void
gtk_dir_selection_dir_activate (GtkTreeView       *tree_view,
				 GtkTreePath       *path,
				 GtkTreeViewColumn *column,
				 gpointer           user_data)
{
  ...
  filename = get_real_filename (filename, TRUE);
  gtk_dir_selection_populate (fs, filename, FALSE, FALSE);
  g_free (filename);
}
...
static void
gtk_dir_selection_populate (GtkDirSelection *fs,
			     gchar            *rel_path,
			     gboolean          try_complete,
			     gboolean          reset_entry)
{
      ...
      if (cmpl_updated_text (cmpl_state)[0])
        {
	      ...
	      gchar* dir_name = g_strdup (cmpl_updated_text (cmpl_state));
              ...
              gtk_dir_selection_populate (fs, dir_name, TRUE, TRUE);
  ...
  if (!did_recurse)
	...
	{
	  gtk_dir_selection_update_history_menu (fs, cmpl_reference_position (cmpl_state));
	}
...
static void
gtk_dir_selection_abort (GtkDirSelection *fs)
{
  gchar err_buf[256];
...
/**
 * gtk_dir_selection_set_select_multiple:
 * @filesel: a #GtkDirSelection
 * @select_multiple: whether or not the user is allowed to select multiple
 * files in the file list.
 *
 * Sets whether the user is allowed to select multiple files in the file list.
 * Use gtk_dir_selection_get_selections () to get the list of selected files.
 **/
void
gtk_dir_selection_set_select_multiple (GtkDirSelection *filesel,
...
/**
 * gtk_dir_selection_get_select_multiple:
 * @filesel: a #GtkDirSelection
 *
 * Determines whether or not the user is allowed to select multiple files in
 * the file list. See gtk_dir_selection_set_select_multiple().
 *
 * Return value: %TRUE if the user is allowed to select multiple files in the
 * file list
 **/
gboolean
gtk_dir_selection_get_select_multiple (GtkDirSelection *filesel)
{
  GtkTreeSelection *sel;

  g_return_val_if_fail (GTK_IS_DIR_SELECTION (filesel), FALSE);
...
static void
gtk_dir_selection_file_changed (GtkTreeSelection *selection,
				 gpointer          user_data)
{
  GtkDirSelection *fs = GTK_DIR_SELECTION (user_data);
  GPtrArray *new_names;
...
static void
gtk_dir_selection_dir_changed (GtkTreeSelection *selection,
				 gpointer          user_data)
{
  GtkDirSelection *fs = GTK_DIR_SELECTION (user_data);
  GPtrArray *new_names;
  ...
  if (index != -1)
	...
	g_free (fs->last_selected);
      ...
      gtk_entry_set_text (GTK_ENTRY (fs->selection_entry), str);
...
/**
 * gtk_dir_selection_get_selections:
 * @filesel: a #GtkDirSelection
 *
 ...
 * gtk_dir_selection_get_filename() would return.
 *
 ...
 **/
gchar **
gtk_dir_selection_get_selections (GtkDirSelection *filesel)
{
  GPtrArray *names;
  gchar **selections;
  ...
  filename = g_strdup (gtk_dir_selection_get_filename (filesel));
			 ...
			 CompletionState *cmpl_state)
{
  ...
  poss = attempt_dir_completion (cmpl_state);
...
static PossibleCompletion*
attempt_dir_completion (CompletionState *cmpl_state)
{
  ...
  if (dir->cmpl_index == dir->sent->entry_count)
      ...
      else
	{
	  ...
	  return attempt_dir_completion (cmpl_state);
	}
    }
