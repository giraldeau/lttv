...
{
  cpu_PropertiesText *properties = (cpu_PropertiesText*)hook_data;
  cpu_DrawContext *draw_context = (cpu_DrawContext*)call_data;
  ...
  switch(properties->cpu_position.x) {
    case POS_START:
      x = draw_context->cpu_drawinfo.start.x;
      switch(properties->cpu_position.y) {
        case OVER:
          offset = &draw_context->cpu_drawinfo.start.offset.over;
          x += draw_context->cpu_drawinfo.start.offset.over;
          y = draw_context->cpu_drawinfo.y.over;
          break;
        case MIDDLE:
          offset = &draw_context->cpu_drawinfo.start.offset.middle;
          x += draw_context->cpu_drawinfo.start.offset.middle;
          y = draw_context->cpu_drawinfo.y.middle;
          break;
        case UNDER:
          offset = &draw_context->cpu_drawinfo.start.offset.under;
          x += draw_context->cpu_drawinfo.start.offset.under;
          y = draw_context->cpu_drawinfo.y.under;
          break;
      }
      /* verify if there is enough space to draw */
      if(unlikely(x + width <= draw_context->cpu_drawinfo.end.x)) {
        enough_space = TRUE;
        *offset += width;
      }
      break;
    case POS_END:
      x = draw_context->cpu_drawinfo.end.x;
      switch(properties->cpu_position.y) {
        case OVER:
          offset = &draw_context->cpu_drawinfo.end.offset.over;
          x += draw_context->cpu_drawinfo.end.offset.over;
          y = draw_context->cpu_drawinfo.y.over;
          break;
        case MIDDLE:
          offset = &draw_context->cpu_drawinfo.end.offset.middle;
          x += draw_context->cpu_drawinfo.end.offset.middle;
          y = draw_context->cpu_drawinfo.y.middle;
          break;
        case UNDER:
          offset = &draw_context->cpu_drawinfo.end.offset.under;
          x += draw_context->cpu_drawinfo.end.offset.under;
          y = draw_context->cpu_drawinfo.y.under;
          break;
      }
      /* verify if there is enough space to draw */
      if(unlikely(x - width >= draw_context->cpu_drawinfo.start.x)) {
        enough_space = TRUE;
        *offset -= width;
      }
      break;
...
{
  cpu_PropertiesIcon *properties = (cpu_PropertiesIcon*)hook_data;
  cpu_DrawContext *draw_context = (cpu_DrawContext*)call_data;
  gboolean retval;
  ...
  {
    *(value.v_pointer) = icon_info = g_new(cpu_IconStruct,1);
...
{
  cpu_PropertiesLine *properties = (cpu_PropertiesLine*)hook_data;
  cpu_DrawContext *draw_context = (cpu_DrawContext*)call_data;
  ...
  x_begin = draw_context->cpu_drawinfo.start.x;
  x_end = draw_context->cpu_drawinfo.end.x;
...
{
  cpu_PropertiesArc *properties = (cpu_PropertiesArc*)hook_data;
  cpu_DrawContext *draw_context = (cpu_DrawContext*)call_data;
...
{
  cpu_PropertiesBG *properties = (cpu_PropertiesBG*)hook_data;
  cpu_DrawContext *draw_context = (cpu_DrawContext*)call_data;
  ...
  gdk_draw_rectangle(draw_context->drawable, draw_context->gc,
          TRUE,
          draw_context->cpu_drawinfo.start.x,
          draw_context->cpu_drawinfo.y.over,
          draw_context->cpu_drawinfo.end.x - draw_context->cpu_drawinfo.start.x,
          draw_context->cpu_drawinfo.y.under - draw_context->cpu_drawinfo.y.over);
