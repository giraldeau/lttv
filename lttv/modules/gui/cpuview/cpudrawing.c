...
GdkColor cpu_drawing_colors[NUM_COLORS] =
{ /* Pixel, R, G, B */
  { 0, 0, 0, 0 }, /* COL_BLACK */
...
static gboolean
cpu_expose_ruler( GtkWidget *widget, GdkEventExpose *event, gpointer user_data );

static gboolean
cpu_expose_vertical_ruler( GtkWidget *widget, GdkEventExpose *event, gpointer user_data );

static gboolean
cpu_motion_notify_ruler(GtkWidget *widget, GdkEventMotion *event, gpointer user_data);

static gboolean
cpu_motion_notify_vertical_ruler(GtkWidget *widget, GdkEventMotion *event, gpointer user_data);
 ...
 */
void cpu_drawing_data_request(cpuDrawing_t *drawing,
      gint x, gint y,
      gint width,
      gint height)
{
...
void cpu_drawing_data_request_begin(EventsRequest *events_request, LttvTracesetState *tss)
{
  g_debug("Begin of data request");
  CpuControlFlowData *cfd = events_request->viewer_data;
...
void cpu_drawing_chunk_begin(EventsRequest *events_request, LttvTracesetState *tss)
{
...
void cpu_drawing_request_expose(EventsRequest *events_request,
                            LttvTracesetState *tss,
                            LttTime end_time)
{
  CpuControlFlowData *cfd = events_request->viewer_data;
  cpuDrawing_t *drawing = cfd->drawing;
 ...
 */
static gboolean
cpu_configure_event( GtkWidget *widget, GdkEventConfigure *event,
    gpointer user_data)
{
   ...
   * executed.
   */
  //lttvwindow_get_time_window(drawing->cpu_control_flow_data->mw,
  //      &drawing->cpu_control_flow_data->time_window);
...
/*modified for cpugram, if width is not changed, GArray is valid so
 just redraw, else recalculate all.(event request again)*/

//enabled again for cpugram:
      if(drawing->pixmap)
...
//          drawing->height);
    ...
    //cpu_drawing_data_request(drawing, &drawing->pixmap, 0, 0,
 ...
 /*     cpu_drawing_data_request(drawing,
                           drawing->damage_begin,
                           0,
                           drawing->damage_end - drawing->damage_begin,
                           drawing->height);*/
    }
//modified for cpugram
       
  if(widget->allocation.width == drawing->width) 
  {
    
    drawing->height = widget->allocation.height;
    cpugram_show( drawing->cpu_control_flow_data,0,
	drawing->cpu_control_flow_data->number_of_process->len);
  }
  else
  { 
    drawing->width = widget->allocation.width;
    ...
    g_array_set_size (drawing->cpu_control_flow_data->number_of_process,
                      	 widget->allocation.width);
    cpu_request_event( drawing->cpu_control_flow_data,drawing->damage_begin
			,drawing->damage_end - drawing->damage_begin);
  }
  return TRUE;
}
...
static gboolean
cpu_expose_event( GtkWidget *widget, GdkEventExpose *event, gpointer user_data )
{
  cpuDrawing_t *drawing = (cpuDrawing_t*)user_data;

  CpuControlFlowData *cpu_control_flow_data =
      (CpuControlFlowData*)g_object_get_data(
                G_OBJECT(widget),
  ...
  LttTime current_time = 
      lttvwindow_get_current_time(cpu_control_flow_data->tab);
...
#if 0
  copy_pixmap_to_screen(cpu_control_flow_data->process_list,
                        widget->window,
                        widget->style->fg_gc[GTK_WIDGET_STATE (widget)],
                        event->area.x, event->area.y,
                        event->area.width, event->area.height);
...
static gboolean
cpu_after_expose_event( GtkWidget *widget, GdkEventExpose *event, gpointer user_data )
{
  //g_assert(0);
...
/* mouse click */
static gboolean
cpu_button_press_event( GtkWidget *widget, GdkEventButton *event, gpointer user_data )
{
  CpuControlFlowData *cpu_control_flow_data =
      (CpuControlFlowData*)g_object_get_data(
                G_OBJECT(widget),
  ...
  if(event->button == 1)
  {
    ...
    cpu_convert_pixels_to_time(drawing->width, (guint)event->x,
        time_window,
        &time);

    lttvwindow_report_current_time(cpu_control_flow_data->tab, time);
...
/*
 //Viewer's vertical scroll bar is already omitted, not needed for cpugram.
static gboolean
...
cpuDrawing_t *cpu_drawing_construct(CpuControlFlowData *cpu_control_flow_data)
{
  cpuDrawing_t *drawing = g_new(cpuDrawing_t, 1);
  
  drawing->cpu_control_flow_data = cpu_control_flow_data;
  ...
  drawing->gc = NULL;
  /* 
  ///at this time not necessary for cpugram
  drawing->hbox = gtk_hbox_new(FALSE, 1);
 
  drawing->viewport = gtk_viewport_new(NULL, cpu_control_flow_data->v_adjust);
  drawing->scrollbar = gtk_vscrollbar_new(cpu_control_flow_data->v_adjust);
  ...
  g_object_set_data_full(
      ...
      "cpu_Link_drawing_Data",
      drawing,
      (GDestroyNotify)cpu_drawing_destroy);
...
//        -1);
  ...
  g_signal_connect (G_OBJECT(drawing->drawing_area),
        "configure_event",
        G_CALLBACK (cpu_configure_event),
        (gpointer)drawing);
 
  g_signal_connect (G_OBJECT(drawing->ruler),
        "expose_event",
        G_CALLBACK(cpu_expose_ruler),
        (gpointer)drawing);
  ...
  g_signal_connect (G_OBJECT(drawing->ruler),
        "motion-notify-event",
        G_CALLBACK(cpu_motion_notify_ruler),
        (gpointer)drawing);

 
  g_signal_connect (G_OBJECT(drawing->vertical_ruler),
        "expose_event",
        G_CALLBACK(cpu_expose_vertical_ruler),
        (gpointer)drawing);

  g_signal_connect (G_OBJECT(drawing->vertical_ruler),
        "motion-notify-event",
        G_CALLBACK(cpu_motion_notify_vertical_ruler),
        (gpointer)drawing);

/*//not necessary for cpuram.
  g_signal_connect (G_OBJECT(drawing->drawing_area),
        "size-allocate",
  ...
  g_signal_connect (G_OBJECT(drawing->drawing_area),
        "expose_event",
        G_CALLBACK (cpu_expose_event),
        (gpointer)drawing);

  g_signal_connect_after (G_OBJECT(drawing->drawing_area),
        "expose_event",
        G_CALLBACK (cpu_after_expose_event),
        (gpointer)drawing);

  g_signal_connect (G_OBJECT(drawing->drawing_area),
        "button-press-event",
        G_CALLBACK (cpu_button_press_event),
        (gpointer)drawing);
 ...
 /// gtk_widget_show(drawing->hbox);
  ...
  gdk_colormap_alloc_colors(colormap, cpu_drawing_colors, NUM_COLORS, FALSE,
                            TRUE, success);
  
  drawing->gc =
    gdk_gc_new(GDK_DRAWABLE(main_window_get_widget(cpu_control_flow_data->tab)->window));
  drawing->dotted_gc =
    gdk_gc_new(GDK_DRAWABLE(main_window_get_widget(cpu_control_flow_data->tab)->window));

  gdk_gc_copy(drawing->gc,
      main_window_get_widget(cpu_control_flow_data->tab)->style->black_gc);
  gdk_gc_copy(drawing->dotted_gc,
      main_window_get_widget(cpu_control_flow_data->tab)->style->white_gc);
...
void cpu_drawing_destroy(cpuDrawing_t *drawing)
{
  g_info("cpu_drawing_destroy %p", drawing);

  /* Free the colors */
  GdkColormap* colormap = gdk_colormap_get_system();

  gdk_colormap_free_colors(colormap, cpu_drawing_colors, NUM_COLORS);
  
  // Do not unref here, cpuDrawing_t destroyed by it's widget.
  ...
  if(drawing->pixmap)
        gdk_pixmap_unref(drawing->pixmap);
  g_free(drawing);
  g_info("cpu_drawing_destroy end");
}

 GtkWidget *cpu_drawing_get_drawing_area(cpuDrawing_t *drawing)
{
  return drawing->drawing_area;
}

 GtkWidget *cpu_drawing_get_widget(cpuDrawing_t *drawing)
{
  return drawing->vbox;
}

 void cpu_drawing_draw_line( cpuDrawing_t *drawing,
      GdkPixmap *pixmap,
      guint x1, guint y1,
      guint x2, guint y2,
      GdkGC *GC)
...
void cpu_drawing_clear(cpuDrawing_t *drawing,guint clear_from,guint clear_to)
{ 
  
  CpuControlFlowData *cfd = drawing->cpu_control_flow_data;
 ...
 /* 
  ...
  rectangle_pixmap(cfd->process_list,
      ...
      -1);*/
  //instead, this is added for cpugram
...
void drawing_insert_square(cpuDrawing_t *drawing,
        guint y,
        guint height)
{
...
void drawing_remove_square(cpuDrawing_t *drawing,
        guint y,
        guint height)
{
  GdkPixmap *pixmap;
...
void cpu_drawing_update_ruler(cpuDrawing_t *drawing, TimeWindow *time_window)
{
  GtkRequisition req;
  GdkRectangle rect;
...
{
  cpuDrawing_t *drawing = (cpuDrawing_t*)user_data;
  TimeWindow time_window = lttvwindow_get_time_window(drawing->cpu_control_flow_data->tab);
  gchar text[255];
 ...
 void cpu_drawing_update_vertical_ruler(cpuDrawing_t *drawing)//, TimeWindow *time_window)
{
  GtkRequisition req;
  GdkRectangle rect;
...
{
  cpuDrawing_t *drawing = (cpuDrawing_t*)user_data;
  CpuControlFlowData *cpu_cfv = drawing->cpu_control_flow_data;
  gchar text[255];
  ...
  snprintf(text, 255, "%.1f", (float)cpu_cfv->max_height);
  ...
  snprintf(text, 255, "%.1f",(float) cpu_cfv->max_height/2.0);
  ...
  LttTime current_time = 
      lttvwindow_get_current_time(cpu_cfv->tab);
  TimeWindow time_window =
            lttvwindow_get_time_window(cpu_cfv->tab);
	...
	(ltt_time_compare(current_time, time_end) <= 0))
  {
     ...
     guint max_height=cpu_cfv ->max_height;
     guint x;
     cpu_convert_time_to_pixels(
                    time_window,
                    current_time,
                    drawing->width,
                    &x);
  //   if(x_test<cpu_cfv->number_of_process->len)

     {
     	events_at_currenttime = 
		&g_array_index(cpu_cfv->number_of_process,guint,x);
