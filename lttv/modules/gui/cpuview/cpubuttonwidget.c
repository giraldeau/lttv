...
extern void cpugram_show(CpuControlFlowData *cpucontrol_flow_data,
...
{
  CpuControlFlowData *cpu_cfd =  (CpuControlFlowData *)user_data;
  //cpu_cfd->vertical_ratio =cpu_cfd->vertical_ratio * (1.0/2.0);
  if(cpu_cfd->max_height>1)
  {
  	cpu_cfd->max_height /= 2;
   //just redraw.horizontal scale is not changed so Array's data are valid.
  	cpugram_show(cpu_cfd ,0,cpu_cfd->number_of_process->len);
  }
  else
	g_warning("Zoom more than 1 event is impossible");
	
  cpu_drawing_update_vertical_ruler(cpu_cfd->drawing);//, TimeWindow *time_window);
  return 0;
}
                   ...
                   gpointer user_data )
{
  ...
  gint i=1,x;
  guint maximum;
  maximum =g_array_index(cpu_cfd->number_of_process,guint,i);
  for (i=1; i < cpu_cfd->number_of_process-> len ;i++)
  {
  	x=g_array_index(cpu_cfd->number_of_process,guint,i);
	maximum=MAX(x,maximum);
  }
  if (maximum >0)
  {
  	cpu_cfd->max_height=maximum;
        cpugram_show (cpu_cfd,0,cpu_cfd->number_of_process->len);
  }
  cpu_drawing_update_vertical_ruler(cpu_cfd->drawing);
...
ButtonWidget *cpu_buttonwidget_construct(CpuControlFlowData *cpucontrol_flow_data)
{
  ...
  buttonwidget->cpu_control_flow_data = cpucontrol_flow_data;
...
/* Pack and show all widgets */
  ...
  gtk_box_pack_end (GTK_BOX (buttonwidget->vbox1),buttonwidget->buttonFit, TRUE, FALSE, 0);
    ...
    g_signal_connect (G_OBJECT (buttonwidget ->buttonP), "clicked",
		G_CALLBACK (gplus), (gpointer)cpucontrol_flow_data);
    g_signal_connect (G_OBJECT ( buttonwidget->buttonM), "clicked",
		G_CALLBACK (gMinus), (gpointer)cpucontrol_flow_data);
    g_signal_connect (G_OBJECT ( buttonwidget->buttonFit), "clicked",
		G_CALLBACK (gFit), (gpointer)cpucontrol_flow_data);
...
void cpu_rectangle_pixmap (GdkGC *gc,
